...
BEGIN
	...
	convert(nvarchar(64), hashbytes('SHA2_256', cast(Patients.PatientID  as nvarchar(36))), 2)PatientIDHash,
		...
		THEN
		(CASE 
					WHEN  Exits.ExitDate IS NOT NULL and Exits.ExitReason<>'DIED' and Exits.EffectiveDiscontinuationDate > EOMONTH(DATEADD(mm,-2,GETDATE()))  THEN 'V'--When a TO and LFTU has an discontinuationdate > Last day of Previous month 
                    ...
                    WHEN  Exits.ExitDate IS NOT NULL and Exits.ExitReason<>'DIED' and  LastPatientEncounter.NextAppointmentDate > EOMONTH(DATEADD(mm,-2,GETDATE()))  THEN 'V'--When a TO and LFTU has an discontinuationdate > Last day of Previous month 
                    ...
                    When Exits.ExitDate IS NOT NULL and Exits.ExitReason<>'DIED' and Exits.ExitDate >EOMONTH(DATEADD(mm,-2,GETDATE()))  THEN 'V'
					When Exits.ExitDate >EOMONTH(DATEADD(mm,-2,GETDATE())) and Exits.ExitReason='DIED' THEN 'V'
					...
					WHEN ART.startARTDate> DATEADD(s,-1,DATEADD(mm, DATEDIFF(m,0,GETDATE()),-30)) THEN 'NP'-- When StartARTDate is after Last Day of Previous EOM 
					WHEN EOMONTH(DATEADD(mm,-2,GETDATE())) < ISNULL(LastPatientEncounter.NextAppointmentDate,ART.ExpectedReturn)-- When last day of previous month is less than TCA
					OR DATEDIFF( dd, ISNULL(LastPatientEncounter.NextAppointmentDate,ART.ExpectedReturn), EOMONTH(DATEADD(mm,-2,GETDATE()))) <=30 THEN 'V'-- Date diff btw TCA  and LAst day of Previous month-- must not be late by 30 days

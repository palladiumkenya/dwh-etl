...
BEGIN
	...
	DISTINCT 
		...
		THEN
		(CASE 
					WHEN  LatestExits.ExitDate IS NOT NULL and LatestExits.ExitReason<>'DIED' and LatestExits.EffectiveDiscontinuationDate > EOMONTH(DATEADD(mm,-2,GETDATE()))  THEN 'V'--When a TO and LFTU has an discontinuationdate > Last day of Previous month 
					WHEN  LatestExits.ExitReason<>'DIED' and LatestExits.EffectiveDiscontinuationDate between  DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE())-2, 0) and DATEADD(MONTH, DATEDIFF(MONTH, -2, GETDATE())-1, -1)  THEN SUBSTRING(LatestExits.ExitReason,1,1)--When a TO and LFTU has an discontinuationdate during the reporting Month 
          WHEN  LatestExits.ExitDate IS NOT NULL and LatestExits.ExitReason<>'DIED' and  Latestexits.ReEnrollmentDate > LatestExits.EffectiveDiscontinuationDate   THEN 'V'
          WHEN  LatestExits.ExitDate IS NOT NULL and LatestExits.ExitReason<>'DIED' and   LatestExits.ExitDate < Latestexits.ReEnrollmentDate    THEN 'V'
          ...
          WHEN  LatestExits.ExitDate IS NOT NULL and LatestExits.ExitReason<>'DIED' and  LastPatientEncounter.NextAppointmentDate > EOMONTH(DATEADD(mm,-2,GETDATE()))  THEN 'V'--When a TO and LFTU has an discontinuationdate > Last day of Previous month 
					When LatestExits.ExitDate IS NOT NULL and LatestExits.ExitReason<>'DIED' and LastPatientEncounter.NextAppointmentDate between DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE())-2, 0) and DATEADD(MONTH, DATEDIFF(MONTH, -2, GETDATE())-1, -1) THEN 'V'
          When LatestExits.ExitDate IS NOT NULL and LatestExits.ExitReason<>'DIED' and LatestExits.ExitDate >EOMONTH(DATEADD(mm,-2,GETDATE()))  THEN 'V'
					When LatestExits.ExitDate >EOMONTH(DATEADD(mm,-2,GETDATE())) and LatestExits.ExitReason='DIED' THEN 'V'
					When LatestExits.ExitDate <EOMONTH(DATEADD(mm,-2,GETDATE())) and LatestExits.ExitReason='DIED' THEN SUBSTRING(LatestExits.ExitReason,1,1)
					When LatestExits.ExitReason='DIED' Then SUBSTRING(LatestExits.ExitReason,1,1)
					...
					WHEN  ISNULL(LastPatientEncounter.NextAppointmentDate,ART.ExpectedReturn) > EOMONTH(DATEADD(mm,-2,GETDATE()))   Then 'V'   -- When last day of previous month is less than TCA
					When  DATEDIFF( dd, ISNULL(LastPatientEncounter.NextAppointmentDate,ART.ExpectedReturn), EOMONTH(DATEADD(mm,-2,GETDATE()))) <=30 THEN 'V'-- Date diff btw TCA  and LAst day of Previous month-- must not be late by 30 days
					WHEN DATEDIFF( dd, ISNULL(LastPatientEncounter.NextAppointmentDate,ART.ExpectedReturn), EOMONTH(DATEADD(mm,-2,GETDATE()))) >30 THEN 'uL'--Date diff btw TCA  and Last day of Previous month

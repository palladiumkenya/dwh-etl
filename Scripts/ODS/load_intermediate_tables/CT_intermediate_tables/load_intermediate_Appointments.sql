...
ARTOutcomesCompuation as (
    ...
    When   DATEDIFF(day, ARTOutcomesCompuation.ExpectedNextAppointmentDate, ARTOutcomesCompuation.LastEncounterDate) >= 91 and ARTOutcomesCompuation.ExpectedNextAppointmentDate <>'1900-01-01' and  ARTOutcomesCompuation.ARTOutcome='uL' Then 'Still IIT'
    -- when ARTOutcomesCompuation.ARTOutcome='uL'  then 'StillIIT'
    When   DATEDIFF(day, ARTOutcomesCompuation.ExpectedNextAppointmentDate, ARTOutcomesCompuation.LastEncounterDate) >= 91 and ARTOutcomesCompuation.ExpectedNextAppointmentDate <>'1900-01-01' and  ARTOutcomesCompuation.ARTOutcome <>'uL' Then 'IIT and RTT beyond 30 days'
...
--where  last_exit_as_of_date.exitReason   in ('COMPLETED','discontinue','ENDPEPPREP','ENDPMTCT','ENDTREAT','HIV Negative','HIV-','OTHER','OTHER NON-CODED','PREPLOWRISK','PREPNOADH','Toxicity, drug','Toxicity, drug','Treatment complete','UNCONF','Unknown','') 
--and ExpectedNextAppointmentDate <>'1900-01-01'

    )
 
    Insert into ODS.dbo.Intermediate_Appointments
   select * from Summary 

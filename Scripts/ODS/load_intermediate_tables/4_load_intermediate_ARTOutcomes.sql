...
                      WHEN  Exits.ExitDate IS NOT NULL and Exits.ExitReason<>'DIED' and Exits.EffectiveDiscontinuationDate > EOMONTH(DATEADD(mm,-1,GETDATE()))  THEN 'V'--When a TO and LFTU has an discontinuationdate > Last day of Previous month                     
					...
					WHEN LastPatientEncounter.NextAppointmentDate IS NULL and ART.ExpectedReturn IS NULL THEN 'NV'                END
            )        ELSE 'FV' END 
    AS ARTOutcome,       cast (Patients.SiteCode as nvarchar) As SiteCode,        Patients.Emr,         Patients.Project
    ...
    LEFT JOIN ODS.dbo.CT_PatientStatus Exits   ON  Patients.PatientPK=Exits.PatientPK  and Patients.Sitecode=Exits.Sitecode
      WHERE  ART.startARTDate IS NOT NULL    ),    LatestUpload AS (    select        cast (SiteCode as nvarchar)As SiteCode ,        Max(DateRecieved) As DateUploaded
     from DWAPICentral.dbo.FacilityManifest
      group by SiteCode
    ),    LatestVisits AS (        Select        distinct sitecode,         Max(Visitdate) As SiteAbstractionDate
         from ODS.dbo.CT_PatientVisits
         group by SiteCode
    )    Select            ARTOutcomes.PatientID,            ARTOutcomes.PatientPK,            ARTOutcomes.startARTDate,            YEAR(ARTOutcomes.startARTDate) AS Cohort,            ARTOutcomes.ExitReason,            ARTOutcomes.ExitDate,            ARTOutcomes.LastEncounterDate,            ARTOutcomes.NextAppointmentDate,            ARTOutcomes.ARTOutcome,            ARTOutcomes.SiteCode,            ARTOutcomes.Emr,            ARTOutcomes.Project,            LatestUpload.DateUploaded,            LatestVisits.SiteAbstractionDate,            cast(getdate() as date) as LoadDate

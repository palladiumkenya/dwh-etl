BEGIN
	 ...
	 DECLARE	@MaxVisitDate_Hist		DATETIME,
		...
		SELECT @VisitDate = MAX(VisitDate) FROM [DWAPICentral].[dbo].[PatientVisitExtract]WITH (NOLOCK)  where visitdate <= Getdate() 		
		   ...
		   --truncate table [ODS].[dbo].[CT_PatientVisits]
			MERGE [ODS].[dbo].[CT_PatientVisits] AS a
				USING(SELECT   P.[PatientCccNumber] AS PatientID, P.[PatientPID] AS PatientPK,F.[Name] AS FacilityName, F.Code AS SiteCode,PV.[VisitId] VisitID,PV.[VisitDate] VisitDate
						...
						WHERE p.gender!='Unknown' and visitdate <= Getdate()) AS b 
						ON(

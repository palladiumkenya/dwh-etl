BEGIN ;
    WITH cte
         AS (SELECT DISTINCT P.patientpid,
                             PS.patientid,
                             F.code,
                             PS.exitdate,
                             PS.exitreason,
                             ...
                             Row_number()
               OVER (
                 partition BY P.patientpid, PS.patientid, F.code, PS.exitdate
               , PS.exitreason, p.lastvisit
                 ORDER BY PS.exitdate DESC) Row_Num
             FROM   [DWAPICentral].[dbo].[patientextract] P WITH (nolock)
                    INNER JOIN [DWAPICentral].[dbo].[patientstatusextract]PS
                               WITH (
                               nolock
                               )
                            ON PS.[patientid] = P.id
                               AND PS.voided = 0
                    INNER JOIN [DWAPICentral].[dbo].[facility] F (nolock)
                    ...
                    INNER JOIN (SELECT P.patientpid,
                                ...
                                FROM   [DWAPICentral].[dbo].[patientextract] P
                                       ...
                                       nolock)
                                       INNER JOIN
[DWAPICentral].[dbo].[patientstatusextract]PS
WITH
(nolock)
        ON PS.[patientid] = P.id
           AND PS.voided = 0
...
DELETE pv
FROM   [DWAPICentral].[dbo].[patientstatusextract] (nolock) PV
...
WHERE  Row_Num > 1;
    ...
    SELECT @MaxExitDate_Hist = Max(maxexitdate)
    FROM   [ODS].[dbo].[ct_patientstatus_log] (nolock);

    SELECT @ExitDate = Max(exitdate)
    FROM   [DWAPICentral].[dbo].[patientstatusextract] WITH (nolock);

    INSERT INTO [ODS].[dbo].[ct_patientstatus_log]
                (maxexitdate,
                 loadstartdatetime)
    VALUES     (@ExitDate,
                Getdate());
    ...
    using(SELECT DISTINCT P.[patientcccnumber]     AS PatientID,
                          ...
                          PS.[recorduuid]
          FROM   [DWAPICentral].[dbo].[patientextract] P WITH (nolock)
                 INNER JOIN [DWAPICentral].[dbo].[patientstatusextract]PS WITH (
                            nolock)
                         ON PS.[patientid] = P.id
                 INNER JOIN [DWAPICentral].[dbo].[facility] F (nolock)
                         ON P.[facilityid] = F.id
                 ...
                 INNER JOIN (SELECT P.patientpid,
                                    F.code,
                                    exitdate,
                                    Max(Cast(Ps.created AS DATE))MaxCreated
                             FROM   [DWAPICentral].[dbo].[patientextract] P WITH
                                    (
                                    nolock)
                                    INNER JOIN
[DWAPICentral].[dbo].[patientstatusextract]PS
...
b.[effectivediscontinuationdate],
...
a.[voided] = b.[voided];
    ...
    INSERT INTO [ODS].[dbo].[ct_patientstatuscount_log]
                ([sitecode],
                 [createddate],
                 [patientstatuscount])
    SELECT sitecode,

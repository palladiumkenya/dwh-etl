...
BEGIN
		...
		source_CD4 as (
			...
			from ODS.dbo.CT_PatientBaselines(NoLock) as baselines
			left join ODS.dbo.CT_Patient(NoLock) as patient on patient.PatientPK = baselines.PatientPK
			and patient.SiteCode = baselines.SiteCode
			left join ODS.dbo.Intermediate_LastPatientEncounter(NoLock) as last_encounter on last_encounter.PatientPK = baselines.PatientPK
